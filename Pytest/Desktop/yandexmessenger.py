...
url = 'https://download.messenger.yandex.ru/desktop/latest?platform=win'
YAMESS_INSTALLER_PATH = r"C:\YaMess.exe"
YAMESS_EXE_PATH = r'C:\Users\user02\AppData\Local\Programs\chats\Yandex Messenger.exe'
YAMESS_VERSION_FILE = r'C:\yamess_version.txt'
FILE_TO_SEND1 = r"C:\files\Yamess.pdf"
#FILE_TO_SEND2 = r"C:\files\file2.pdf"
#FILE_TO_SEND3 = r"C:\files\file3.pdf"
destination = YAMESS_INSTALLER_PATH
name_mess = 'YandexMessenger'
install_args = ["/S"]
...
def messeges_yamess():
    ...
    search_field = pyautogui.locateCenterOnScreen('screens/ya_icon.png', confidence=0.8)
    if search_field:
    ...
    message_field = pyautogui.locateCenterOnScreen('screens/ya_field.png', confidence=0.8)
    if message_field:
        pyautogui.click(message_field)
        time.sleep(1)
        message_sender = Messages("This is test messages_yamess")
...
def check_database():
    try:
        ...
        if db.message_exists(messages, 'This is test messages_yamess'):
        ...
        if db.message_exists(files, 'Yamess.pdf'):
...
def main():
    ...
    try:
        ...
        # 2. Установка      
        if check_installer_exists(YAMESS_INSTALLER_PATH):

            installer = AppInstaller(
                installer_path=YAMESS_INSTALLER_PATH,
            ...
            while not os.path.exists(YAMESS_EXE_PATH):
            ...
            if os.path.exists(YAMESS_EXE_PATH):
                logging.info(f"{name_mess} успешно установлен: {YAMESS_EXE_PATH}")
            else:
            ...
            logging.error(f"Установочный файл не найден: {YAMESS_INSTALLER_PATH}")
            return
        ...
        if os.path.exists(YAMESS_EXE_PATH):
            ...
            logging.error(f"{name_mess} не найден после установки")
            return

        # 4. Запуск
        yamess = StartApp(YAMESS_EXE_PATH)
        yamess.run()
        ...
        close_app_by_name("Yandex Messenger.exe")
